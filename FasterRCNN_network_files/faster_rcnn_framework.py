    ...
    def forward(self, images, targets=None):
        ...
        Returns:
            result (list[BoxList] or dict[Tensor]): the output from the model.
                During training, it returns a dict[Tensor] which contains the losses(包含损失的dict[Tensor]).
        ...
        if self.training:
            ...
            for target in targets:  # 进一步判断传入的target的boxes参数是否符合规定
                boxes = target["boxes"]
                # isinstance用于检查一个对象是否是某个类（或某些类）的实例,返回布尔值（True 或 False）
        ...
        images, targets = self.transform(images, targets)  # 对图像进行预处理，标准化处理和最大最小缩放
...
class FasterRCNN(FasterRCNNBase):
    ...
    def __init__(self, backbone, num_classes=None,
                 ...
                 rpn_nms_thresh=0.5,  # rpn中进行nms处理时使用的iou阈值
