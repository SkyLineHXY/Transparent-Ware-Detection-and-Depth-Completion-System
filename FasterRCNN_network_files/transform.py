...
class GeneralizedRCNNTransform(nn.Module):
    ...
    def batch_images(self, images, size_divisible=32):
        ...
        实现步骤：
            1. 计算一个batch中所有图片中的最大channel, height, width
            2. 将height向上调整到stride的整数倍
            3. 将width向上调整到stride的整数倍
            4. 创建shape为batch_shape且值全部为0的tensor
            5. 将输入images中的每张图片复制到新的batched_imgs的每张图片中，对齐左上角，保证bboxes的坐标不变
